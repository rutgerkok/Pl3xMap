...
public final class ShowCommand extends Pl3xMapCommand {
    ...
    public void register() {
        this.commandManager.registerSubcommand(builder ->
                builder.literal("show")
                        .meta(MinecraftExtrasMetaKeys.DESCRIPTION, MiniMessage.miniMessage().deserialize(Lang.SHOW_COMMAND_DESCRIPTION))
    ...
    private void executeShow(final @NonNull CommandContext<CommandSender> context) {
        ...
        if (!this.plugin.playerManager().hidden(target)) {
            Lang.send(sender, Lang.PLAYER_NOT_HIDDEN, Placeholder.unparsed("player", target.getName()));
            return;
        }

        this.plugin.playerManager().show(target, true);
        Lang.send(sender, Lang.PLAYER_SHOWN, Placeholder.unparsed("player", target.getName()));
    }
