...
public final class CancelRenderCommand extends Pl3xMapCommand {
    ...
    public void register() {
        this.commandManager.registerSubcommand(builder ->
                builder.literal("cancelrender")
                        ...
                        .meta(MinecraftExtrasMetaKeys.DESCRIPTION, MiniMessage.miniMessage().deserialize(Lang.CANCEL_RENDER_COMMAND_DESCRIPTION))
    ...
    private void executeCancelRender(final @NonNull CommandContext<CommandSender> context) {
        ...
        if (!world.isRendering()) {
            Lang.send(sender, Lang.RENDER_NOT_IN_PROGRESS, Placeholder.unparsed("world", world.name()));
            return;
        }

        Lang.send(sender, Lang.CANCELLED_RENDER, Placeholder.unparsed("world", world.name()));
        world.stopRender();
